...
目前 Jenkins 主機預設允許區網前端 `http://192.168.0.177:8180`。若主機 IP 或前端連接埠改變，請在 Build with Parameters 將 `FOODLEDGER_CORS_ALLOWED_ORIGIN` 改成實際前端 Origin。前端 Docker build 的 `FOOD_LEDGER_API_BASE_URL` 也必須使用瀏覽器可連線的後端區網 URL，不能使用 `localhost`。
